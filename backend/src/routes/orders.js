...
async function createDbOrder({userId,body,paymentMethod="COD"}){return withTransaction(async conn=>{
 ...
 const normalized=input.map(x=>{const p=map.get(String(x.productId||x.id||""));if(!p){const e=new Error("One or more products are unavailable");e.status=400;throw e;}const q=Math.max(1,Math.min(100,Number(x.quantity)||1));if(Number(p.stock)<q&&paymentMethod!=="RAZORPAY"){const e=new Error(`${p.name} has insufficient stock`);e.status=400;throw e;}return {p,q,x};});
...
router.post("/",async(req,res,next)=>{try{const result=await createDbOrder({userId:req.user?.id,body:req.body,paymentMethod:req.body.paymentMethod||"COD"});res.status(201).json({order:result});}catch(e){next(e)}});
